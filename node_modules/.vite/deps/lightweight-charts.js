...
function merge(dst, ...sources) {
  for (const src of sources) {
    for (const i in src) {
      ...
      if ("object" !== typeof src[i] || dst[i] === void 0) {
        dst[i] = src[i];
      } else {
        merge(dst[i], src[i]);
      }
...
var PaneRendererMarks = class extends MediaCoordinatesPaneRenderer {
  constructor() {
    super(...arguments);
  ...
  _internal__drawImpl({ context: ctx }) {
    ...
    const draw = (radius2) => {
      ...
      for (let i = visibleRange.to - 1; i >= visibleRange.from; --i) {
        ...
        ctx.arc(point._internal_x, point._internal_y, radius2, 0, Math.PI * 2);
      }
      ctx.fill();
    };
...
function walkLine(renderingScope, items, lineType, visibleRange, barWidth, styleGetter, finishStyledArea2) {
  ...
  if (visibleRange.to - visibleRange.from < 2) {
    ...
    finishStyledArea2(ctx, currentStyle, item1, item2);
    return;
  }
  const changeStyle = (newStyle, currentItem2) => {
    finishStyledArea2(ctx, currentStyle, currentStyleFirstItem, currentItem2);
    ctx.beginPath();
  ...
  if (currentStyleFirstItem !== currentItem || currentStyleFirstItem === currentItem && lineType === 1) {
    finishStyledArea2(ctx, currentStyle, currentStyleFirstItem, currentItem);
  }
}
var curveTension = 6;
...
var PaneRendererAreaBase = class extends MediaCoordinatesPaneRenderer {
  constructor() {
    super(...arguments);
    this._internal__data = null;
  }
...
var PaneRendererLineBase = class extends MediaCoordinatesPaneRenderer {
  constructor() {
    super(...arguments);
    this._internal__data = null;
  }
...
function drawSquare(ctx, centerX, centerY, size3) {
...
function drawArrow(up, ctx, centerX, centerY, size3) {
...
function drawCircle(ctx, centerX, centerY, size3) {
...
var SeriesMarkersRenderer = class extends MediaCoordinatesPaneRenderer {
  constructor() {
    super(...arguments);
    this._private__data = null;
...
var PriceScale = class {
  ...
  _private__formatPrice(price, fallbackFormatter) {
    if (this._private__localizationOptions.priceFormatter === void 0) {
...
var FormattedLabelsCache = class {
  ...
  _internal_format(tickMark) {
    ...
    const cacheKey = time._internal_businessDay === void 0 ? new Date(time._internal_timestamp * 1e3).getTime() : new Date(Date.UTC(time._internal_businessDay.year, time._internal_businessDay.month - 1, time._internal_businessDay.day)).getTime();
...
var TimeScaleVisibleRange = class {
...
var TimeScale = class {
  ...
  _internal_formatDateTime(timeScalePoint) {
    ...
    return this._private__dateTimeFormatter._internal_format(new Date(timeScalePoint._internal_time._internal_timestamp * 1e3));
  }
  ...
  _private__formatLabelImpl(tickMark) {
    ...
    if (this._private__options.tickMarkFormatter !== void 0) {
...
function sourceTopPaneViews(source, pane) {
  return source._internal_topPaneViews !== void 0 ? source._internal_topPaneViews(pane) : [];
}
...
function fillWeightsForPoints(sortedTimePoints, startIndex = 0) {
  ...
  for (let index = startIndex; index < sortedTimePoints.length; ++index) {
    ...
    const currentDate = new Date(currentPoint._internal_time._internal_timestamp * 1e3);
    if (prevDate !== null) {
  ...
  if (startIndex === 0 && sortedTimePoints.length > 1) {
    ...
    const approxPrevDate = new Date((sortedTimePoints[0]._internal_time._internal_timestamp - averageTimeDiff) * 1e3);
    sortedTimePoints[0]._internal_timeWeight = weightByTime(new Date(sortedTimePoints[0]._internal_time._internal_timestamp * 1e3), approxPrevDate);
  }
}
...
var DataLayer = class {
  ...
  _internal_updateSeriesData(series, data) {
    ...
    if (lastSeriesTime !== void 0 && time._internal_timestamp < lastSeriesTime._internal_timestamp) {
